...
public class FTClient {
    public static void uploadParams(FTModel model) {
        ...
        ClientPacketHandler.sendC2S(new TrackingParamsPayload(Objects.requireNonNull(Minecraft.getInstance().player).getUUID(), model.eyeR, model.eyeL, model.mouth, model.isFlat));
    }
    ...
    public static void writeSync(FTModel model) {
        ...
        try {
            ...
            buf.readBytes(data);
            ClientPacketHandler.sendC2S(new TrackingUpdatePayload(Objects.requireNonNull(Minecraft.getInstance().player).getUUID(), data));
        } finally {
            buf.release();
        }
    }
