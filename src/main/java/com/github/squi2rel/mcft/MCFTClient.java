...
public class MCFTClient {
	...
	public static void onInitializeClient() {
        ...
        ClientPacketHandler.registerS2C(ConfigPayload.ID, ConfigPayload.CODEC, p -> MC.execute(() -> {
            if (!checkVersion(p.version())) {
                Objects.requireNonNull(MC.player).sendSystemMessage(Component.translatable("mcft.message.versionmismatch", MCFT.version, p.version()));
                return;
            }
            fps = p.fps();
            if (!connected) MCFT.LOGGER.info(I18n.get("mcft.logging.connected"));
            connected = true;
    ...
    public static void update() {
        ...
        uuidToModel.entrySet().removeIf(entry -> Objects.requireNonNull(MC.getConnection()).getPlayerInfo(entry.getKey()) == null);
        if (configScreen && MC.screen == null) {
