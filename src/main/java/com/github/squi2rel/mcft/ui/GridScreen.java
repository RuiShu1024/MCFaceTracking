...
public abstract class GridScreen extends Screen {
    ...
    public GridScreen(Component title, int gridLength, int drawSize) {
        super(title);
    ...
    protected Selection getSelection() {
        if (freeDrag) {
            ...
            int x1 = Mth.clamp((int) (selectedStartX / cellSize), 0, gridLength - 1);
            int y1 = Mth.clamp((int) (selectedStartY / cellSize), 0, gridLength - 1);
            int x2 = Mth.clamp((int) (selectedEndX / cellSize), 0, gridLength - 1);
            int y2 = Mth.clamp((int) (selectedEndY / cellSize), 0, gridLength - 1);
    ...
    protected void drawGrid(GuiGraphicsExtractor context, int x, int y) {
        ...
        for (int i = 0; i <= gridLength; i++) {
            context.horizontalLine(x, x + drawSize, y + i * cellSize, 0xFF000000);
            context.verticalLine(x + i * cellSize, y, y + drawSize, 0xFF000000);
        }

        if(selectedStartX != -1 && selectedEndX != -1) {
            if (freeDrag) {
                context.fill(
                        Mth.clamp(gridX + selectedStartX, gridX, gridX + drawSize),
                        Mth.clamp(gridY + selectedStartY, gridY, gridY + drawSize),
                        Mth.clamp(gridX + selectedEndX, gridX, gridX + drawSize),
                        Mth.clamp(gridY + selectedEndY, gridY, gridY + drawSize),
                        0x55FF0000
                );
            } else {
                int x1 = Mth.clamp(selectedStartX / cellSize, 0, gridLength - 1) * cellSize;
                int y1 = Mth.clamp(selectedStartY / cellSize, 0, gridLength - 1) * cellSize;
                int x2 = Mth.clamp(selectedEndX / cellSize, 0, gridLength - 1) * cellSize;
                int y2 = Mth.clamp(selectedEndY / cellSize, 0, gridLength - 1) * cellSize;
                context.fill(
    ...
    protected void drawSelection(GuiGraphicsExtractor context, Selection selection, int color) {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (allowSelect && event.button() == GLFW.GLFW_MOUSE_BUTTON_LEFT && mouseX >= gridX && mouseX <= gridX + drawSize && mouseY >= gridY && mouseY <= gridY + drawSize) {
            selecting = true;
    ...
    public boolean mouseDragged(MouseButtonEvent event, double deltaX, double deltaY) {
        if (selecting && event.button() == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            updateSelection(event.x(), event.y(), false);
            return true;
        }
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        if (event.button() == GLFW.GLFW_MOUSE_BUTTON_LEFT) selecting = false;
    ...
    public class WidgetGroup {
        ...
        public <T extends AbstractWidget> T add(T e) {
    ...
    public static class SettingsSlider<T extends Number> extends AbstractSliderButton {
        ...
        public SettingsSlider(int x, int y, int width, int height, T value, T min, T max, Consumer<T> callback, Function<T, String> textProvider, Function<Double, T> converter) {
            ...
            this.value = (Mth.clamp(value.doubleValue(), min.doubleValue(), max.doubleValue()) -  min.doubleValue()) / (max.doubleValue() - min.doubleValue());
        ...
        public void applyValue() {
            callback.accept(converter.apply(Mth.lerp(Mth.clamp(this.value, 0.0, 1.0), this.min, this.max)));
        }

        @Override
        protected void updateMessage() {
            this.setMessage(Component.literal(textProvider.apply(converter.apply(Mth.lerp(Mth.clamp(this.value, 0.0, 1.0), this.min, this.max)))));
        }

        public void setValue(T number) {
            double newValue = (Mth.clamp(number.doubleValue(), min, max) - min) / (max - min);
