...
public class EyeTrackingRect extends TrackingRect {
    ...
    public void validate(boolean init) {
        if (init) {
            ...
            lid.validate(true);
            inner.validate(true);
            ball.w = Mth.clamp(ball.w, 0, 8 - ball.x);
            ball.h = Mth.clamp(ball.h, 0, 8 - ball.y);
            lid.w = Mth.clamp(lid.w, 0, 8 - lid.x);
            lid.h = Mth.clamp(lid.h, 0, 8 - lid.y);
            inner.w = Mth.clamp(inner.w, 0, 8 - inner.x);
            inner.h = Mth.clamp(inner.h, 0, 8 - inner.y);
        } else {
            percent = Mth.clamp(percent, 0, 1);
            rawPos.x = Mth.clamp(rawPos.x, -2, 2);
            rawPos.y = Mth.clamp(rawPos.y, -2, 2);
        }
    }
